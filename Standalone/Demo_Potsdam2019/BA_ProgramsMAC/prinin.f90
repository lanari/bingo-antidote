...
!+++++
      IF (NGUESS(IS).GT.0) THEN
       WRITE (UNIT=scr,FMT=165)
       WRITE (UNIT=out,FMT=165)
  165  FORMAT (/,' Initials for minimization:')
        IF (NGUESS(IS).GT.0) THEN
         WRITE (UNIT=scr,FMT='(1X,A)') SOLNAM(IS)
         WRITE (UNIT=out,FMT='(1X,A)') SOLNAM(IS)
         WRITE (UNIT=scr,FMT='(5X,100A16)') &
               (NAME(EM(IS,II)),II=1,NEND(IS))
         WRITE (UNIT=out,FMT='(5X,100A16)') &
               (NAME(EM(IS,II)),II=1,NEND(IS))
         DO I=1,NGUESS(IS)
          WRITE (UNIT=scr,FMT='(1X,100F16.10)') &
                (GUESS(IS,I,II),II=1,NEND(IS))
          WRITE (UNIT=out,FMT='(1X,100F16.10)') &
                (GUESS(IS,I,II),II=1,NEND(IS))
         END DO
        END IF
      END IF

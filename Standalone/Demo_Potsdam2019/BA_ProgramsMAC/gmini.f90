...
!-----    init : initial conposition from ini_SOLNAM-file
...
!******************************
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 I,II,IIX,I001,K,LOO2,CLCOD,ADDCOD,J,I1,I2
...
!DC      IF (GTOT.LE.GTEST.OR.DISTAMAX.LT.DXMIN) THEN
      ...
      IF ((.NOT.AFAIL.AND.GTOT.LE.GTEST).OR.SAMEAS) THEN
      TRY=TRY+1
      CLCOD=0
...
!-------------------------------------------------------------------
!CDC9NOV2012
!following is to reduce NMAX. Does not improve minimization!
!at the moment all SUGUSE(i) should be 1
!-----
!      DO K=1,NMAX
!      WRITE (UNIT=scr,FMT='(I4,I4,I4)') NUMMER(K),EMCODE(K),SUGUSE(K)
!      END DO
!-----
!      GOTO 811
!      K=NUN+1
!  810 IF (K.GT.NMAX) GOTO 811
!      IF (SUGUSE(I).NE.1) THEN
!        I1=K
!        I2=NMAX
!        CALL COLCHG(I1,I2)
...
!-----
      ...
      DO 665,I=1,NUN
  665 G(I)=1D20
!-- adding structural endmembers
!-- apparently something is wrong. left out for the moment
!      DO 670,IS=1,NSOL
!      IF (NSTREM(IS).GT.0) THEN
!      DO 671,II=1,NSTREM(IS)
!      DO 675,I=1,NEND(IS)
!  675 XXSC(I)=STREM(IS,II,I)
...
!CDC
!      IF (TEST.LT.0.0D0) THEN
!      WRITE (UNIT=6,FMT='(''adding structural endmember'')')
!      WRITE (UNIT=out,FMT='(''adding structural endmember'')')
!      END IF
!CDC
!!add SUGCOD!
!      CALL NEWPH(IS)
!c      SUGG(NMAX)=-SUGG(NMAX)
!      WRITE (6,1010) (X(NMAX,I),I=1,NUN)
!      WRITE (out,1010) (X(NMAX,I),I=1,NUN)
! 1010 FORMAT (20(F10.5,2X))
!  671 CONTINUE
!      END IF
!  670 CONTINUE
!-- end adding structural endmembers
      END IF
      CALL FULLRED
      CODE=.TRUE.
...
!==
 ...
 1017 FORMAT (' old ass:  ',7(I5,12X),100(/8X,7(I5,12X)))
      ELSE
...
!--
      ...
      IF (IS.EQ.I2) THEN
      DO 410,J=1,NEND(IS)
!2  410 XXSC(J)=(XEM(I,J)+XEM(II,J))/2.0D0
...
!==
!==
!---- add close to stem (does not really improve)
!==
!      DO 420,I=1,NUN
!      IF (NUMMER(I).GT.0) THEN
!      IF (EMSOL(NUMMER(I)).GT.0) THEN
!--
!      IF (TEST.LT.0.0D0) THEN
!      WRITE (UNIT=6,FMT='('' add close to stem'')')
!      WRITE (UNIT=out,FMT='('' add close to stem'')')
!      END IF
!--
!      IS=EMSOL(NUMMER(I))
!      DO 422,J=1,NEND(IS)
!  422 XXEM(J)=1D-10
!      XXEM(EMNR(NUMMER(I)))=1.0D0-DBLE(NEND(IS)-1)*1D-10
!!add SUGCOD!!!!
!      CALL NEWPH(IS)
!      END IF
!      END IF
!  420 CONTINUE
!---- end add close to stem
!==
!---
      DISTAMAX=0.0D0
      SUGLOOP=SUGNR+1
...
!==
      DO I=1,NEND(IS)
        ...
        ANUMBER=I
!
!      WRITE (UNIT=scr,FMT='('' now marmin:'',3I6)') I001,I002,ANUMBER
!      WRITE (UNIT=out,FMT='('' now marmin:'',3I6)') I001,I002,ANUMBER
      IF (TEST.LT.0.0D0) THEN
...
!CCCCC      IF (NNEGEM(IS).GT.0) I1=I1-NSTREM(IS)
!CCCCC
!!!!!!      IF (NNEGEM(IS).GT.0.AND.I1.GT.0) I1=0
!--
!      I1=-NEND(IS)
      I2=LOCMIN(IS)
!--
...
!      IF (LOCMIN(IS).GE.1.AND.LOMIN1(IS).GE.1) I1=-NEND(IS)
...
!===
      DO 510,START=I1,I2
      I002=START
!cdcFeb11      IF (I1.LT.0.AND.DXSCAN.EQ.1.0D0.AND.START.EQ.0 &
!cdcFeb11      .AND.GTOT.GT.GTEST) GOTO 510
!cdcFeb11
!********
      CALL MARMIN(I001,I002,ANUMBER,XXSC)
!********
...
!
!===
!----- use initial guess from ini_...
!===
!      DO I=1,NEND(IS)
!       XXSC(I)=0.0D0
!      END DO
!
      IF (NGUESS(IS).GT.0) THEN
       DO I=1,NGUESS(IS)
        DO J=1,NEND(IS)
         XXSC(J)=GUESS(IS,I,J)
        END DO
...
!==
      IF (TEST.LT.0.0D0) THEN
      WRITE (UNIT=6,FMT='('' minisug = '',i5)') MINISUG(IS)
      WRITE (UNIT=out,FMT='('' minisug = '',i5)') MINISUG(IS)
      END IF
!==
  500 CONTINUE
      RETURN
...
!******************************
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 IS,I,CODE,N,PROB,PR01
      REAL*8 XXSC(EMAX),GSC,FF,MUE2(EMAX)
!-----
      PROB=0
...
!==
        IF (TEST.LT.0.0D0) THEN
          WRITE (6,1002) SUGNR+1,SOLNAM(IS)
          WRITE (out,1002) SUGNR+1,SOLNAM(IS)
 1002     FORMAT (' spacetest failed: ',I6,2X,A)
        END IF
!==
      END IF
!====
...
!-----START = 999:Initial gess from file ini_...
!-----
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      include 'files.cmn'
...
!-----half scan for weird solutions
      GSOL=1D30
      IF (START.EQ.0.AND.NNEGEM(IS).GT.0) THEN
!
       FRAC=4
       FRAC=1
       DO I=1,FRAC-1
...
!-
  ...
  718   FORMAT (' minimum for this step: ',I6,8X,'G = ',1PE12.5)
      END IF
!******     

...
!******************************
      ...
      include 'files.cmn'
!-----END OF COMMON VARIABLES
      INTEGER*4 I,II,I001,IS,NGEM,ARR(EMAX)
      CHARACTER*500 CH001
      CHARACTER*16 GEM(EMAX),CH16
      REAL*8 XGEM,F1,F2
!=====
      DO IS=1,NSOL
       NGUESS(IS)=0
...
!=====
!---- meaning of '+' changed in order to be compatible with
!---- Doug Tinkham's coding of Landau Phases.
!---- So: using '+' will exclude the phase from becoming stable
!---- For the old meaning of '+', use '-'
...
!---- volume function (Holland and Powell 2011)
      ...
      END IF
!---- eduester18:
!---- volume function (Stixrude and Lithgow-Bertelloni 2011)
      IF (RECODE.EQ.'VS1') THEN
      READ (UNIT=REC,FMT='(5X,3D15.0)',ERR=999) VPA,VPB,VLN
      VPA=VPA*1000.0D0
      VOLVO=12
      VO3=.FALSE.
      VO1=.FALSE.
      VO2=.FALSE.
      END IF
      IF (RECODE.EQ.'VS2') THEN
      READ (UNIT=REC,FMT='(5X,5D15.0)',ERR=999) D1,D2,D3,D4,VAA
      VOLVO=12
      VO3=.FALSE.
      VO1=.FALSE.
      VO2=.FALSE.
      END IF
      IF (RECODE.EQ.'VS3') THEN
      READ (UNIT=REC,FMT='(5X,2D15.0)',ERR=999) SHM0, SHMP
      SHM0=SHM0*1000.0D0
      VELOTYP=3
      VOLVO=12
      VO3=.FALSE.
      ...
      END IF
!---- eduester18:      
!---- Landau (Stixrude and Lithgow-Bertelloni 2011)
      IF (RECODE.EQ.'LA2') THEN
      READ (UNIT=REC,FMT='(5X,3D15.0)',ERR=999) TKRI,SMA,VTA
      VOLVO=12
      VO3=.FALSE.
      VO1=.FALSE.
      VO2=.FALSE.
...
!---- dk0dt for liquids (Holland and Powell 2011)
      ...
      END IF
!---- eduester18:
!---- shear modulus (Connolly and Kerrick, 2002)
      IF (RECODE.EQ.'SMC') THEN
        VELOTYP=1
        READ (UNIT=REC,FMT='(5X,3D15.0)',ERR=999) SHM0, SHMP, SHMT
      END IF
!---- eduester18:
!---- shear modulus using Poisson's ratio
!---- (Duesterhoeft, 2014; Nagel et al., 2018submitted)
      IF (RECODE.EQ.'SMD') THEN
        VELOTYP=2
        READ (UNIT=REC,FMT='(5X,2D15.0)',ERR=999) SHM0, SHMP
      END IF
!----      

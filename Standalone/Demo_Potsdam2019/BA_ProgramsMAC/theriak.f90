...
!-----END OF COMMON VARIABLES
      ...
      CHARACTER*500 SYREC,BULKLINE(2)
      CHARACTER*100 XVARI,YVARI
      CHARACTER*80 DATI,ZEITSTRING,TEXT,TEXT1,TEXT2
      REAL*8 WERT1,WERT2
...
!-----END OF COMMON VARIABLES
      ...
      CHARACTER*100 XVARI,YVARI
      CHARACTER*80 DATI,ZEITSTRING,TEXT,TEXT2
      REAL*8 WERT2
      CHARACTER*8 KEY1,KEY2
...
!-----END OF COMMON VARIABLES
      ...
      INFLOW(COMAX),OUTFLOW(COMAX),F1,NNREM(50),X16,PCVREM(50)
       ...
       DO IP=1,NUN2
        ...
        DO IR=1,NREP
         IF (REPHASE(IR).EQ.TEXT) THEN
          ...
          !-- eduester18
          PCVREM(IR)=PCVREM(IR)+VOLPH(IP)*(REFRA(IR))
         END IF
          CH16='nsum_'//REPHASE(IR)
          X16=NNREM(IR)
          CALL SETTBL(CH16,X16)
          !-- eduester18
          CH16='pcVsum_'//REPHASE(IR)
          X16=(PCVREM(IR)/(VOLSOL+PCVREM(IR)-VOLPH(IP)*(REFRA(IR))))*100.0D0
          CALL SETTBL(CH16,X16)
        END DO
       END DO
      END IF

...
! variables used by Doug Tinkham (+k0,dkdt,pth)
      ...
      pkb,vpt
! variables used for Stixrude and Lithgow-Bertelloni 2011, by eduester18
      INTEGER*4 IZAP,IZAP1     
      REAL*8 NR9,C1,C2,C3,C5,C7,DT,QM1,FL,DFL,ITIC,VQ,GVQ,V23, &
      F,DF,D2F,ROOT,THT,THT0,ETHT,ETHT0,LTHT,LTHT0,DD1,DD2,DD3, &
      DTHT,DTHT0,D2THT,D2THT0,DG,DG0,D2G,D2G0,DFC,D2FC, &
      DFT,DFT0,D2FT,D2FT0,PLG,PLGG,PLGG0,TOL,VOLUM0      
!=====
      FGVOL=0.0D0
      FVVOL=0.0D0
      FGDIS=0.0D0
...
!		after Hellfrich and Connolly(2009)  
		
      PTH=(-K0/(K01))*(1.0D0+A0*(T-T0)+A01*(T-T0)*(T-T0))**(-K01) &
          -(-K0/(K01))&
          *(1.0D0-AA*(1.0D0-(1.0D0+BB*PK)**(-CC)))**(K01+1.0D0)*(1.0D0+BB*PK)**(CC+1.0D0)
!       last term describes pressure dependence on thermal pressure (see Duesterhoeft, 2016)
!-----      
...
!--- here LANDAU CALL
      IF (VOLVO.EQ.12.AND.VLN.EQ.1) THEN
      DXP=P/1D5
      PJ=P
      CALL LANDSTX11(PJ,GCH)
      GR = GR+GCH
      !---- now dG/dP
      	IF (VNEED) THEN
        	DG1=GCH
...
!      write(*,*) 'shear ',  SHM(PHMAX+1), SHM0, NAME(IP), IP

      IF (THT.GT.5D0.or.THT0.GT.5D0.and.IZAP1.LT.100) THEN
         WRITE (*,*) 'danger will robinson, danger, danger!!', &
                     NAME(IP),THT0,THT
!                    STPHNAM(NUN2),THT0,THT
         IZAP1 = IZAP1 + 1
      END IF 

!      if (isnan(GR).or.isnan(SHM(II))) then
!         write (*,*) 'Stixrude11: GR or SHM(II) is not a number'
!      end if 
                   
1000  format (/,'**warning ver369** failed to converge at T= ',f8.2,' K' &
            ,' P=',f9.1,'bar',/,' Using Sixtrude EoS.',  &
             ' Phase ',a,' will be destabilized.',/)
1010  format (/,'**warning ver369** will not be reapeated for future ',  &
               'instances of this problem',/)

   
!99    write(*,*) 'T= ', t, 'P= ', p
!      write(*,*) 'END_VOLUME: ', VOLUM
!      write(*,*) 'END_GIBBS: ', GR       
99     END IF
!
!********************************************************************************
...
! eduester18: for Stixrude and Lithgow-Bertelloni, 2011
! pirated from PERPLEX
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
      ...
      DATA c2 /0.1111111111111111111/

!                              13th order expansion good to t ~ 3
!       c1 = 1/3
!     PLG  = t**3*((dlog(t)*c1-c2) - t/8d0 + t**2/120d0     
...
!******************************
! eduester18
!===== Landau order function from Stixrude and Lithgow-Bertelloni 2011.
!===== must be used with volume function VOLVO=12
!===== VS1  VPA, VPB, VLN
!===== LA2  TKRI,      SMA,  VTA
!           Tcrit(T0)  Smax  Vmax
      SUBROUTINE LANDSTX11(PJ,GCH)
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
!-----END OF COMMON VARIABLES
      REAL*8 PJ,GCH,Q40,Q4,Q20,Q2,Q60,Q6, &
      DGLA,TKR,TEF,VMA
!-----       
      VMA=VTA
      TKR=TKRI+(VMA/SMA)*PJ
...
!      Q40=1.0D0-T0/TKRI
      ...
      Q60=Q20*Q40
      Q6=Q2*Q4
!---  as in Stixrude and Lithgow-Bertelloni 2011    
     DGLA=SMA*((TEF-TKR)*Q20+(1.0D0/3.0D0)*TKRI*Q60)
...
!      WRITE (UNIT=6,FMT='(''jetzt gcalc'',2(1PE15.8))') PGAS,T
  ...
  650 VOLUM=VOLUM+FFCOM(I)*VV(ICOM(I))
      ...
      VV(I001)=VOLUM
!********************************************************************************     
!eduester18: shear modulus 

      !IF (PRTLOG(6)) THEN
      !	IF (SHM0.EQ.0.0D0) THEN
      ! 		WRITE(*,*) NAME(II), II, SHM(II), SHM0
      !	END IF
	  !END IF
      
      ! Poisson ratio after Duesterhoeft, 2013
      ...
      IF (VELOTYP.EQ.3) THEN
           SHM(II)=SHM(PHMAX+1)
           ! shear modulus after Stixrude and Lithgow-Bertelloni, 2011
!           WRITE(*,*) "SHEAR_MODULUS 3:  ",NAME(II), II, SHM(II), VPA, SHM0
      ELSE IF (VELOTYP.EQ.1) THEN
           SHM(II)=SHM0+T*SHMT+P*SHMP
           ! shear modulus after Connolly and Kerrick, 2002
!          WRITE(*,*) "SHEAR_MODULUS 1:  ",NAME(II), II, SHM(II)
      ELSE IF (VELOTYP.EQ.2) THEN
            SHM(II)=SHM0
            SHM1(II)=SHMP
            ! Poisson ratio after Duesterhoeft, 2014
!           WRITE(*,*) "SHEAR_MODULUS 2:  ",NAME(II), II, SHM(II)
      END IF
!********************************************************************************      
  600 CONTINUE
      RETURN

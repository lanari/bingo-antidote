...
!CCCC      CALL PRTSTR(1,NMAX)
      CALL FINCLEAN
      END IF
!******************************
!-- eduester18 - call seismic velocities
IF (VELOTYP.NE.0) CALL SWAVE 	
!=====
      RETURN
      END
!-----
...
! by eduester18
      ...
      COMMON /LOSOLID/ ISASOLID
!-----END OF COMMON VARIABLES
      REAL*8 MUH, MUHS, KT, KS, FF, FF1, TAU
      REAL*8 MUH_PH(COMAX),MUH1_PH(COMAX)
      REAL*8 DG0,DG1,DG2,DG3,TVOR,PVOR,DXT,DXP, KS_V,KS_R 
      REAL*8 SHEAR_PH(COMAX), &
      KT_PH(COMAX), KS_PH(COMAX), SVELO_PH(COMAX),PVELO_PH(COMAX), &
      KT0,KS0,KT0_PH(COMAX), &
      KTP_PH(COMAX),KTM_PH(COMAX),ALPH_PH(COMAX), &
      GRUEN_PH(COMAX),NA_PH(COMAX)
      INTEGER*4 I,IPH, NOS, IE, IS
!      REAL*8 KHSM,KHSP,MHSM,MHSP
...
!-----
      ...
      DO I=1,NUN2  ! NUN2 = number of stable phases
        ...
        IF (EMCODE(I) .EQ. 0) THEN
        !means, it is NOT a solid solution, only a normal phase
        !NUMMER = Nummer der Phase in der Datenbank z.B. Oliven ist 81
        
          MUH_PH(I)=SHM(NUMMER(I))  !shear modulus of a SOLID! phase (VOLSOL)          
          MUH1_PH(I)=SHM1(NUMMER(I))! exponent TAU for VELOTYP=2
          !write(*,*) SHM(NUMMER(I)), NUMMER(I), NAME(NUMMER(I))
          
        ELSE
        !means, it is a solid solution
          FF=0.0D0
          FF1=0.0D0
          IS=EMCODE(I)

          DO IE=1,NEND(IS) ! number of endmember of a solution
            FF=FF+SHM(EM(IS,IE))*XEM(I,IE)
            FF1=FF1+SHM1(EM(IS,IE))*XEM(I,IE)
          END DO
          
          MUH_PH(I)=FF            ! shear modulus of a phase
          MUH1_PH(I)=FF1          ! exponent TAU for VELOTYP=2        
        END IF 
        !write(*,*) 'SHEAR ',NN(I), GG(I), STPHNAM(I), SHM(NUMMER(I))*VOLPH(I), FF*VOLPH(I)
      END DO           
!-----
      KS_V=0.0D0
      KS_R=0.0D0
      
      ! Voigt-Reuss-Hill of shearm_solids
      CALL VOIGTREUSS(KS_V,KS_R,MUH_PH)
      MUHS=(KS_V+KS_R)/2.0D0
      ! Voigt-Reuss-Hill of shearm_all
      CALL VOIGTREUSS2(KS_V,KS_R,MUH_PH)
      MUH=(KS_V+KS_R)/2.0D0
!----- calculate bulk modulus KS
      PVOR=P
      TVOR=TC
      CALL SORTOFNURVONPT
      CALL GETKAPPA(KT,KS,KT_PH,KS_PH,GRUEN_PH,ALPH_PH)

      !... new KS optional
      !CALL VOIGTREUSS(KS_V,KS_R,KS_PH)
      !KS=(KS_V+KS_R)/2.0D0
      !...
...
!       
!===== 
  IF (VELOTYP.EQ.1.OR.VELOTYP.EQ.3) THEN
  !----VELOTYP=1: shear modulus (Connolly and Kerrick, 2002)
  !----VELOTYP=3: shear modulus (Stixrude and Lithgow-Bertelloni, 2011)
  !
    SVELO=SQRT(MUH/((SUM(WTPH))/(SUM(VOLPH))))*10D0   !S-Wave velocity of all stable phases incl. 'GASDATA' 
    SSVELO=SQRT(MUHS/(WTSOL/VOLSOL))*10D0 !S-Wave velocity of solids [m/s]--> it's used
    PVELO=SQRT((KS+4.0D0*MUH/3.0D0)/((SUM(WTPH))/(SUM(VOLPH))))*10D0   !P-Wave velocity of all stable
    PPVELO=SQRT((KS+4.0D0*MUHS/3.0D0)/(WTSOL/VOLSOL))*10D0 !P-Wave velocity of solids
        ...
        IF (DRU) THEN
         ...
         DO I=1,NUN2
         	IF (ISASOLID(I)) THEN
                  ...
                  WRITE (6,751) STPHNAM(I),VOLPH(I)/VOLSOL*100, &
                  (3.0D0*KS_PH(I)-2.0D0*MUH_PH(I))/(6.0D0*KS_PH(I)+2.0D0*MUH_PH(I)), &
                  KS_PH(I)/1000D0, MUH_PH(I)/1000D0, &
         	      WTPH(I)/VOLPH(I),SQRT(MUH_PH(I)/(WTPH(I)/VOLPH(I)))*10D0, &
         	      SQRT((KS_PH(I)+4.0D0*MUH_PH(I)/3.0D0)/(WTPH(I)/VOLPH(I)))*10D0

         	END IF
         END DO
         
         WRITE (6,754) (3.0D0*KS-2.0D0*MUHS)/(6.0D0*KS+2.0D0*MUHS), & 
         KS/1000D0 , MUHS/1000D0,WTSOL/VOLSOL,SSVELO,PPVELO
...
!       
!=====
  IF (VELOTYP.EQ.2) THEN
  !----VELOTYP=2: shear modulus using Poisson's ratio
  !----          (Duesterhoeft, 2014; Nagel et al., 2018)  
  !
      !DXP=0.1
      DXP=0.49
      ...
      DO I=1,NUN2
        ...
        CALL SORTOFNURVONPT
        CALL RECHG2(IPH,DG2)
        !--- Volume(T) of phases at P=1bar
        VOLPH2(I)=(DG1-DG2)/(2.0D0*DXP)*10.0D0
        !write(*,*) STPHNAM(I),VOLPH2(I)
        !---
        P=PVOR
        TC=TVOR
        CALL SORTOFNURVONPT        
...
!    P=1
!    CALL SORTOFNURVONPT   
!    CALL GETKAPPA(KT0,KS0,KT0_PH,NA_PH,NA_PH,NA_PH)
!    
!    P=PVOR
!    TC=TVOR
!    CALL SORTOFNURVONPT
! -----DG1 calculate kappa_(P=P+50kbar,T)     
    PVOR=P
    TVOR=TC
    ...
    P=P+50000.0D0!+DXP
    CALL SORTOFNURVONPT   
    CALL GETKAPPA(KT0,KS0,KTP_PH,NA_PH,NA_PH,NA_PH)
    
    P=PVOR
    TC=TVOR
    CALL SORTOFNURVONPT        
! -----DG3 calculate kappa_(P,T)     
!    PVOR=P
!    TVOR=TC
!    
!    !TC=T!-DXT
!    P=P-DXP
!    CALL SORTOFNURVONPT   
!    CALL GETKAPPA(KT0,KS0,KTM_PH,NA_PH,NA_PH,NA_PH)
!    
!    P=PVOR
!    TC=TVOR
!    CALL SORTOFNURVONPT          
!-----
    ...
    DO I=1,NUN2
            ! computes "isothermal" shear modulus(T0,P0) of each phase:    
            !DG0=(1.0D0+T*ALPH_PH(I)*GRUEN_PH(I)*FEMG(I))  &
            !(3D0*KT0_PH(I)-6D0*KT0_PH(I)*MUH0_PH(I))/(2D0+2D0*MUH0_PH(I))
            
            ! computes "isothermal" shear modulus(T,+DXP) of each phase:           
            DG1=(1.0D0+T*ALPH_PH(I)*GRUEN_PH(I)*FEMG(I))  & ! 
            *(3D0*KTP_PH(I)-6D0*KTP_PH(I)*MUH_PH(I))/(2D0+2D0*MUH_PH(I))
            ! computes "isothermal" shear modulus(T,P) of each phase:
            DG2=(1.0D0+T*ALPH_PH(I)*GRUEN_PH(I)*FEMG(I))  &
            *(3D0*KT_PH(I)-6D0*KT_PH(I)*MUH_PH(I))/(2D0+2D0*MUH_PH(I))
            ! computes exponent tau:
            IF (MUH1_PH(I).LT.(-999.89D0)) THEN
            	TAU=(DG1-DG2)/(50000.0D0)-GRUEN_PH(I)
            ELSE
            	TAU=MUH1_PH(I)
            END IF
            ! computes shear modulus of each phase:
            SHEAR_PH(I)=(1.0D0+T*ALPH_PH(I)*GRUEN_PH(I)*FEMG(I)) &
            *((3D0*KT_PH(I)-6D0*KT_PH(I)*MUH_PH(I))/(2D0+2D0*MUH_PH(I)))  &
            *(VOLPH2(I)/VOLPH(I))**(-TAU) !***
            ! computes Poisson's ratio of each phase:
    		MUH_PH(I)=(3D0*KS_PH(I)-2D0*SHEAR_PH(I))/(6D0*KS_PH(I)+2D0*SHEAR_PH(I))
    		! computes seismic velocities of each phase:	
    		SVELO_PH(I)=SQRT(SHEAR_PH(I)/(WTPH(I)/VOLPH(I)))*10.0D0
    		PVELO_PH(I)=SQRT((KS_PH(I)+4.0D0*SHEAR_PH(I)/3.0D0)/(WTPH(I)/VOLPH(I)))*10.0D0
    		!write(*,*) STPHNAM(I),SHEAR_PH(I),SVELO_PH(I),PVELO_PH(I), NN(I),GRUEN_PH(I),DG1,DG0

    END DO 

    ! Voigt-Reuss-Hill of shearm_solids
    CALL VOIGTREUSS(KS_V,KS_R,SHEAR_PH)
    MUHS=((KS_V+KS_R)/2.0D0)    
    ! Voigt-Reuss-Hill of shearm_all
    CALL VOIGTREUSS2(KS_V,KS_R,SHEAR_PH)
    MUH=((KS_V+KS_R)/2.0D0)

    
    SVELO=SQRT(MUH/((SUM(WTPH))/(SUM(VOLPH))))*10D0   !S-Wave velocity of all stable phases incl. 'GASDATA' 
    SSVELO=SQRT(MUHS/(WTSOL/VOLSOL))*10D0             !S-Wave velocity of solids [m/s]--> is used
    PVELO=SQRT((KS+4.0D0*MUH/3.0D0)/((SUM(WTPH))/(SUM(VOLPH))))*10D0
    PPVELO=SQRT((KS+4.0D0*MUHS/3.0D0)/(WTSOL/VOLSOL))*10D0
        ...
        IF (DRU) THEN
         ...
         !       
		 752 FORMAT ( &
      /'  solid phases',11X,'vol%  |',2X, &
      'Poisson ratio  ',2X,'Ks [kbar]',2X,'Mu [kbar]',2X,'density [g/ccm]  |',4X,'s-wave velocity  |',4X,'p-wave velocity' &
      /'  ------------',15X,'  |',7X, &
      '          ',2X,'          ',26X,'   |',4X,'      ',7X, &
      4X,'|',1X,'               ' )
         !====
         DO I=1,NUN2
         	IF (ISASOLID(I)) THEN
         	      WRITE (6,751) STPHNAM(I),VOLPH(I)/VOLSOL*100, MUH_PH(I), KS_PH(I)/1000D0, SHEAR_PH(I)/1000D0, &
         	      WTPH(I)/VOLPH(I),SVELO_PH(I), PVELO_PH(I)
         	      !
                  751 FORMAT (2X,A16,1X,2X,F8.4,'  | ', &
                  6X,F8.4,2X,F10.2,2X,F10.2,5X,F12.6,'  | ',8X,F11.4,' | ',6X,F11.4,' ')
         	END IF
         END DO
         !====
         WRITE (6,754) (3D0*KS-2D0*MUHS)/(6D0*KS+2D0*MUHS),KS/1000D0,MUHS/1000D0,WTSOL/VOLSOL,SSVELO,PPVELO
         !
         754 FORMAT (21X,'--------  | ',6X,'--------', 4X,'--------',4X,'--------',9X,'--------  |',12X, &
         '-------- |',10X,'--------',/ &
         '  total of solids',9X,'100','  | ', &
                  6X,F8.4,2X,F10.2,2X,F10.2,5X,F12.6,'  | ',8X,F11.4,' | ',6X,F11.4,' ')
         !====         
        END IF
        END IF    
! Print HSWBOUNDS:    
      !CALL NSOLID(NOS)
      !CALL HSWBOUNDS(KHSM,KHSP,MHSM,MHSP,KS_PH,SHEAR_PH, NOS)
      !write(*,*) 'KS_HS= ', ((KHSM+KHSP)/2D0)/1000D0, KHSM/1000D0, KHSP/1000D0
      !write(*,*) 'M_HS= ', ((MHSM+MHSP)/2D0)/1000D0, MHSM/1000D0, MHSP/1000D0
      !write(*,*) 'V_HS= ',SQRT(((MHSM+MHSP)/2D0)/(WTSOL/VOLSOL))*10D0 , & 
      !SQRT((((KHSM+KHSP)/2D0)+4D0*((MHSM+MHSP)/2D0)/3D0)/(WTSOL/VOLSOL))*10D0
      !write(*,*) 'V_HS2= ',SQRT(((MHSM+MHSP)/2D0)/(WTSOL/VOLSOL))*10D0 , &
      !SQRT((((KS))+4D0*((MHSM+MHSP)/2D0)/3D0)/(WTSOL/VOLSOL))*10D0  
      !write(*,*) 'POI_HS= ',((3D0*KHSM-2D0*MHSP)/((6D0*KHSM+2D0*MHSP))+ &
      !(3D0*KHSP-2D0*MHSM)/((6D0*KHSP+2D0*MHSM)))/2D0, &
      !(3D0*KHSM-2D0*MHSP)/((6D0*KHSM+2D0*MHSP)), (3D0*KHSP-2D0*MHSM)/((6D0*KHSP+2D0*MHSM))         
  END IF   
...
!******************************
! by eduester18      
! Kt_total, Ks_total, Kt of phase, Ks of phase, Grueneisen parameter, thermal expansion
      SUBROUTINE GETKAPPA(KT_TOT,KS_TOT,KT_PH,KS_PH,LECP_PH,EXPAN_PH)
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      LOGICAL*4 ISASOLID(COMAX)
      COMMON /LOSOLID/ ISASOLID
!-----
      REAL*8 KT_TOT,KS_TOT,KT_PH(COMAX),KS_PH(COMAX),LECP_PH(COMAX)
      REAL*8 DG(-2:2,-2:2), DG_PH(COMAX,-2:2,-2:2)
      REAL*8 DG0,DG1,DG2,DGM1,DGM2,TVOR,PVOR,DXT,DXP, &
      TE0,TE1,TE2,TEM1,TEM2, &
      DV0, DV1,DVM1,DVDPX,DS0,DSP1,DSPM1,DSDP, &
      DST1,DSTM1, DSDT, NNTOT, HH1, HHM1
      REAL*8 DV0_PH(COMAX), DV1_PH(COMAX),DVM1_PH(COMAX),DVDPX_PH(COMAX), &
      DS0_PH(COMAX),DSP1_PH(COMAX),DSPM1_PH(COMAX),DSDP_PH(COMAX), &
      DST1_PH(COMAX),DSTM1_PH(COMAX), DSDT_PH(COMAX), &
      DVDTX_PH(COMAX),DVT1_PH(COMAX),DVTM1_PH(COMAX), EXPAN_PH(COMAX)    
      INTEGER*4 I, II
!-----
      !-----
      ! It's set to 0.1 because of lower sensivity to PVELO, if DX is smaller
      IF (P<50000D0) THEN
        DXP=0.49
      ELSE
        DXP=P/1D5
...
!-----
      ...
      DO I=1,NUN2
           IF (ISASOLID(I)) THEN
           		NNTOT=NNTOT+NN(I) ! Total number of moles
           END IF
           CALL RECHG2(I,DGM1)
           DG_PH(I,0,0)=DGM1/NN(I)
      END DO  
      TE0=T
!-----
      TC=TVOR+DXT
...
!===== System parameter
       ...
       !=====
       KS_TOT=DVDPX+(DSDP*DSDP)/DSDT
       KS_TOT=-DV0/KS_TOT
       KT_TOT=-DV0/DVDPX
!===== Mineral Phase parameter
	  DO I=1,NUN2
       !DG(T-index,P-index)
       DV0_PH(I)=(DG_PH(I,0,1)-DG_PH(I,0,-1))/(2.0D0*DXP)
       DV1_PH(I)=(DG_PH(I,0,2)-DG_PH(I,0,0))/(2.0D0*DXP)
       DVM1_PH(I)=(DG_PH(I,0,0)-DG_PH(I,0,-2))/(2.0D0*DXP)
       
       DVT1_PH(I)=(DG_PH(I,1,1)-DG_PH(I,1,-1))/(2.0D0*DXP)
       DVTM1_PH(I)=(DG_PH(I,-1,1)-DG_PH(I,-1,-1))/(2.0D0*DXP)
       DVDTX_PH(I)=(DVT1_PH(I)-DVTM1_PH(I))/(2.0D0*DXT)

       DVDPX_PH(I)=(DV1_PH(I)-DVM1_PH(I))/(2.0D0*DXP)
       DS0_PH(I)=-(DG_PH(I,1,0)-DG_PH(I,-1,0))/(2.0D0*DXT)
       DSP1_PH(I)=-(DG_PH(I,1,1)-DG_PH(I,-1,1))/(2.0D0*DXT)
       DSPM1_PH(I)=-(DG_PH(I,1,-1)-DG_PH(I,-1,-1))/(2.0D0*DXT)
       DSDP_PH(I)=(DSP1_PH(I)-DSPM1_PH(I))/(2.0D0*DXP)
       DST1_PH(I)=-(DG_PH(I,2,0)-DG_PH(I,0,0))/(2.0D0*DXT)
       DSTM1_PH(I)=-(DG_PH(I,0,0)-DG_PH(I,-2,0))/(2.0D0*DXT)
       DSDT_PH(I)=(DST1_PH(I)-DSTM1_PH(I))/(2.0D0*DXT)
       !=====       
       HH1=DG_PH(I,1,0)+TE1*DST1_PH(I)
       HHM1=DG_PH(I,-1,0)+TEM1*DSTM1_PH(I)
       LECP_PH(I)=(HH1-HHM1)/(2.0D0*DXT)
       EXPAN_PH(I)=(1.0D0/DV0_PH(I))*DVDTX_PH(I)
       !write(*,*) 'alpha_GETKAPPA',EXPAN_PH(I)
       !=====
       KS_PH(I)=DVDPX_PH(I)+(DSDP_PH(I)*DSDP_PH(I))/DSDT_PH(I)
       KS_PH(I)=-DV0_PH(I)/KS_PH(I)
       KT_PH(I)=-DV0_PH(I)/DVDPX_PH(I) 
         
      !=====
      ! volume to molar volume
      DV0_PH(I)=DV0_PH(I)/NN(I)
      !specific molar heat capacity (J/(K*g))
      !LECP_PH(I)=LECP_PH(I)/(WTM(I))
      !thermal Grüneisen parameter
      !write(*,*) KS_PH(I),DV0_PH(I),LECP_PH(I),WTM(I),EXPAN_PH(I),NN(I)
      !LECP_PH(I)=(EXPAN_PH(I)*KS_PH(I))/(LECP_PH(I)*(WTM(I)/(DV0_PH(I))))
      ...
      LECP_PH=LECP_PH/(WTM)
      !thermal Grüneisen parameter
      !write(*,*) 'GK',STPHNAM(I-1),EXPAN_PH(I-1),KS_PH(I-1),LECP_PH(I-1),WTM(I-1),DV0_PH(I-1),VOLM(I-1)
      !LECP_PH=(EXPAN_PH*KS_PH)/(LECP_PH*(WTM/(DV0_PH)))
      LECP_PH=(EXPAN_PH*KS_PH)/(LECP_PH*(WTM/(VOLM/10.0D0)))
!=====
...
!******************************
! by eduester18
! Calculate Voigt-Reuss of solids
      SUBROUTINE VOIGTREUSS(M_V,M_R,M_PH)
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      LOGICAL*4 ISASOLID(COMAX)
      COMMON /LOSOLID/ ISASOLID
!-----
      REAL*8 M_R,M_V,M_PH(COMAX)
      INTEGER*4 I
!-----
      M_R=0.0D0
      M_V=0.0D0
      DO I=1,NUN2
      IF (ISASOLID(I)) THEN
          M_V=M_V+M_PH(I)*VOLPH(I)/VOLSOL
          M_R=M_R+(VOLPH(I)/VOLSOL)/M_PH(I)
      END IF
      END DO
      M_R=M_R**(-1.0D0)
!*****
...
!******************************
! by eduester18
! Calculate Voigt-Reuss of all phases
      SUBROUTINE VOIGTREUSS2(M_V,M_R,M_PH)
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      LOGICAL*4 ISASOLID(COMAX)
      COMMON /LOSOLID/ ISASOLID
...
!-----
      ...
      M_V=0.0D0
      DO I=1,NUN2
          M_V=M_V+M_PH(I)*VOLPH(I)/(SUM(VOLPH))
          M_R=M_R+(VOLPH(I)/SUM(VOLPH))/M_PH(I)
      END DO
      M_R=M_R**(-1.0D0)
!*****
      RETURN
...
!******************************
! by eduester18
! Calculate Hashin-Shtrikman-Walpole bounds 
      SUBROUTINE HSWBOUNDS(KHSM,KHSP,MHSM,MHSP,KS_PH,SHEAR_PH, NOS)     
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      LOGICAL*4 ISASOLID(COMAX)
      COMMON /LOSOLID/ ISASOLID
!-----
      INTEGER*4 I,IPH, NOS
      REAL*8 KHSM,KHSP,MHSM,MHSP,KS_PH(COMAX),SHEAR_PH(COMAX)
      REAL*8 KMIN,KMAX,MMINI,MMAX, ZETMIN, ZETMAX
      REAL*8 KS_PHS(NOS), SHEAR_PHS(NOS), VOLPRO(NOS)
      REAL*8 SKM,SKP,SMM,SMP
!-----
      KHSM=0.0D0
      KHSP=0.0D0
      MHSM=0.0D0
      ...
      IPH=0.0D0
      DO I=1,NUN2
      !CALCULATE VOLUMETRIC PROPORTION
      IF (ISASOLID(I)) THEN
          IPH=IPH+1
          VOLPRO(IPH)=(VOLPH(I)/VOLSOL)
          KS_PHS(IPH)=KS_PH(I)
          SHEAR_PHS(IPH)=SHEAR_PH(I)
      END IF
      END DO            
      
      KMIN=MINVAL(KS_PHS);
	  KMAX=MAXVAL(KS_PHS);
	  MMINI=MINVAL(SHEAR_PHS);
      MMAX=MAXVAL(SHEAR_PHS);
      !write(*,*) KMIN/1000.0D0,KMAX/1000.0D0,MMINI/1000.0D0,MMAX/1000.0D0,NOS
      
	  ! zeta-functions for shear modulus
		ZETMIN=MMINI/6.0D0*(9.0D0*KMIN + 8.0D0*MMINI)/(KMIN + 2.0D0*MMINI);
		ZETMAX=MMAX/6.0D0*(9.0D0*KMAX + 8.0D0*MMAX)/(KMAX + 2.0D0*MMAX);
	  ! Weighted Sums (Lamda and Gamma functions)
		SKM=SUM(VOLPRO/(KS_PHS + (4.0D0*MMINI/3.0D0)));
		SKP=SUM(VOLPRO/(KS_PHS + (4.0D0*MMAX/3.0D0)));
		SMM=SUM(VOLPRO/(SHEAR_PHS + ZETMIN));
		SMP=SUM(VOLPRO/(SHEAR_PHS + ZETMAX));
	  ! clean up
		KHSM=1.0D0/SKM - 4.0D0*MMINI/3.0D0;
		KHSP=1.0D0/SKP - 4.0D0*MMAX/3.0D0;
		MHSM=1.0D0/SMM - ZETMIN;
		MHSP=1.0D0/SMP - ZETMAX;      
!*****
      RETURN
      END
!-----
!******************************
! by eduester18
! Compute number of solids
      SUBROUTINE NSOLID(NOS)
      ! CALCULATE NUMBER OF SOLID
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
      LOGICAL*4 ISASOLID(COMAX)
      COMMON /LOSOLID/ ISASOLID
...
!---- PRTLOG(9): print table (P-T-loop)
      ...
      CALL SETTBL(CH16,X16)
      CH16='shearm_solids'
      X16=((WTSOL/VOLSOL)*SSVELO**(2.0D0))/100000.0D0
      CALL SETTBL(CH16,X16)
      CH16='Ks_solids'
      X16=(PPVELO*PPVELO*(WTSOL/VOLSOL)-(4.0D0/3.0D0)*(WTSOL/VOLSOL)*SSVELO**(2.0D0))/100000.0D0
      CALL SETTBL(CH16,X16)
      !--      
      END IF
!*****
...
!-----
!******************************
      SUBROUTINE RECHG2(I,DGX) ! compute G of single phases
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
!
!      LOGICAL*4 ISASOLID(COMAX)
...
!-----
!******************************
      SUBROUTINE RECHG(DGX) !compute system G
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
!
!      LOGICAL*4 ISASOLID(COMAX)
...
!-----
!******************************
! eduester18: new subroutine, same as RECHG(DGX), but only for solids
      SUBROUTINE RECHGS(DGX)
      IMPLICIT NONE
      INCLUDE 'theriak.cmn'
!
      LOGICAL*4 ISASOLID(COMAX)
...
!++++
       END IF
      !write(*,*) 'RECHGS' ,STPHNAM(I), GGG, DGX 
      END DO
!*****
      RETURN
      END

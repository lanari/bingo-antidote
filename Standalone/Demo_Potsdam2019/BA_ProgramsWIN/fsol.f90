...
!-----**hopo liquid**
      ...
      NAME(8)='H2O.liq'
      END IF
!-----**hopo liquid + spear phosphates**
      IF (SOLNAM.EQ.'LIQsp') THEN
      N=11
      NAME(1)='Silica8.liq'
      NAME(2)='albite.liq'
      NAME(3)='K-feldspar.liq'
...
!     coded by Thorsten Nagel
      ...
      END IF
!
!-----basaltic melt: Jennings and Holland 2015
!     coded by Thorsten Nagel
      IF (SOLNAM.EQ.'PERMELT') THEN
      N=9
      NAME(1)='qtzL_sol'
      NAME(2)='diL_sol'
      NAME(3)='jdL_sol'
...
!-----**basaltic melt: Jennings and Holland 2015
      IF (SOLNAM.EQ.'PERMELT') THEN
      MODELL='basaltic melt: Jennings and Holland (2015)'
      END IF
!*****
      RETURN
      END
...
!********************************
      ...
      A1,A2,A3,A4,A5,LNG1,LNG2,LNG3,LNG4,LNG5,FF,YY,XX, &
      ...
      CAM3,MGM3,FEM3,NAM3,SIM1,CAM1,NAM1,SIM2, &
...
!-----** taken from tc-6NCKFMASHTO.txt in thermocalc_340i
      ...
      CAM2=X(6)
      ALT1=(X(4)+X(5))/2.0D0
      SIT1=X(1)+X(2)+X(3)+X(4)/2.0D0+X(5)/2.0D0+X(6)
      A(1)=MGM1*MGM2*SIT1**0.5D0
      A(2)=FEM1*FEM2*SIT1**0.5D0
...
!
      ...
      FEM2=X(5)+X(7)
      MGM1=(X(4)+X(6)+X(7))/2.0D0
      FEM1=X(5)/2.0D0
      ALM1=X(1)*5.0D0/6.0D0+X(2)+X(3)
      SIM1=X(1)/6.0D0+(X(4)+X(5)+X(6)+X(7))/2.0D0
!-
      A(1)=NAM3*MGM2**2.0D0*ALM1**(2.5D0)*SIM1**(0.5D0)*3.86393D0
      A(2)=CAM3*MGM2**2.0D0*ALM1**3.0D0
      A(3)=MGM3*MGM2**2.0D0*ALM1**3.0D0
      A(4)=MGM3*(MGM2**2.0D0)*(MGM1**(1.5D0))*SIM1**(1.5D0)*8.0D0
      A(5)=FEM3*FEM2**2.0D0*FEM1**(1.5D0)*SIM1**(1.5D0)*8.0D0
      A(6)=FEM3*MGM2**2.0D0*MGM1**(1.5D0)*SIM1**(1.5D0)*8.0D0
      A(7)=FEM3*FEM2**2.0D0*MGM1**(1.5D0)*SIM1**(1.5D0)*8.0D0
      END IF
!
!-------Cpx Holland et al. 2013
!       coded by Thorsten Nagel
...
!
      ...
      MGM2=X(1)+X(5)
      FEM2=X(2)+X(3)
      SIT1=X(1)+X(2)+X(3)+X(4)+X(5)/2.0D0
      ALT1=X(5)/2.0D0
!
      A(1)=MGM1*MGM2*SIT1**0.5D0
      A(2)=FEM1*FEM2*SIT1**0.5D0
...
!
      ...
      FEM1=X(4)+X(5)
      NAM1=X(6)
      MGM2=X(3)/2.0D0+X(5)/2.0D0
      FEM2=X(4)/2.0D0
      ALM2=X(1)+X(2)+X(6)/2.0D0
      SIM2=(X(3)+X(4)+X(5)+X(6))/2.0D0
!
      A(1)=MGM1*ALM2
      A(2)=CAM1*ALM2
...
!                             M         F         A
!    (1) - qtz_im             -         Si2       -
!    (2) - diopside_im        CaMg      Si2       -
!    (3) - jadeite_im         -         AlSi2     Na
!    (4) - catsch_im          
!    (5) - forst_im
!    (6) - fayal_im
!    (7) - hemat_im
...
!
      IF (SOLNAM.EQ.'PERMELT') THEN
      A(1)=X(1)
      A(2)=X(2)**2.0D0*(X(2)+2.0D0*X(5))/ &
      ((X(5)+X(6)+X(2))**2.0D0)
      A(3)=X(3)
      A(4)=X(4)
      A(5)=(X(2)+2.0D0*X(5))**2.0D0*(X(5)+X(6)) &
      /((X(5)+X(6)+X(2))**2.0D0)*0.25D0
      A(6)=(X(6)**2.0D0)*(X(5)+X(6))/((X(5)+X(6)+X(2))**2.0D0)
      A(7)=X(7)
      A(8)=X(8)
      END IF
!----
